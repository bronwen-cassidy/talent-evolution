...
public class HibernatePortfolioDao extends HibernateCrudAdaptor implements IPortfolioDao {
    ...
    public Collection getContentTypes(String nodeType, Collection contentTypePermits) {
        ...
        String query = "from com.zynap.talentstudio.organisation.portfolio.ContentType type where type.type=:type and type.id IN (" + contentTypes.toString() + ") order by upper(type.label)";
        return getHibernateTemplate().findByNamedParam(query, "type", nodeType);
    }
    ...
    public Collection getAllContentTypes(String action, Collection contentTypePermits) {
        ...
        String query = "from com.zynap.talentstudio.organisation.portfolio.ContentType type where type.id IN (" + contentTypePermitString.toString() + ") order by upper(type.label)";
